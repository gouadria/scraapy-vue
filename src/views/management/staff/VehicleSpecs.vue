...
export default defineComponent({
  ...
  methods: {
    async fetchApi() {
      ...
      try {
        const { data } = await this.$axios.get('/api/inventory/staff/vehicle-specs/pending/')
    ...
    async handleApproveRequest() {
      ...
      try {
        const response = await this.$axios.post(
          `/api/inventory/staff/vehicle-specs/approve/${this.items[0].id}/`,
          { ...this.items[0] }
        )
    ...
    async handleRejectItem(reason: string) {
      ...
      try {
        await this.$axios.post(`/api/inventory/staff/vehicle-specs/reject/${this.rejectItem.id}/`, {
          staff_note: reason
        })
